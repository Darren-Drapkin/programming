...
#the string of players names from the set returned by  Player.at
# this is called from the acceptor to give a list of players names in a
#given room
	 #players is a list of touples

	 def roster(players)do
	 # break off the first touple
	 [player|other_players] = players
	 # put the string at the head of the first touple onto the output string
	 # and recurse down the remainder of the list
	 case(length(players))do
		0-> "and none other"
		1-> " " <> elem(player,0) 
		_ ->elem(player,0)<> " "<>roster(other_players) 
	 end
	 end
  ...
  @doc ~S"""
  State the contents of a player's bag.
...
# quit a player from the game
      ...
      def do_quit({player_name,socket})do
